...
function activate(context) {
    ...
    context.subscriptions.push(vscode.workspace.onDidChangeTextDocument(event => {
        if (event.document.languageId !== "senior")
            return;
        const diagnostics = analyzeDocument(event.document);
        diagnosticCollection.set(event.document.uri, diagnostics);
    }));
    context.subscriptions.push(vscode.commands.registerCommand("senior.createVariable", createVariableCommand));
    context.subscriptions.push(vscode.commands.registerCommand("senior.fixBackslashes", fixBackslashesCommand));
    context.subscriptions.push(vscode.languages.registerCodeActionsProvider("senior", { provideCodeActions }, { providedCodeActionKinds: [vscode.CodeActionKind.QuickFix] }));
}
...
function createDiagnostic(lineIndex, lineText, message) {
...
function createVariableCommand(document, varName) {
...
function fixBackslashesCommand(document, lineNumber) {
...
function provideCodeActions(document, range, context, token) {
    ...
    for (const diag of context.diagnostics) {
        ...
        if (msg.includes("não foi definida com 'Definir'")) {
            const varName = (_a = msg.match(/"(.+?)"/)) === null || _a === void 0 ? void 0 : _a[1];
            if (!varName)
                continue;
            const fix = new vscode.CodeAction(`Criar variável "${varName}"`, vscode.CodeActionKind.QuickFix);
            fix.command = {
        ...
        if (msg.includes("barras não escapadas")) {
            const fix = new vscode.CodeAction(`Escapar barras da string`, vscode.CodeActionKind.QuickFix);
            fix.command = {
